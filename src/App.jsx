...
/* ---------- Auth ---------- */
// Logo "G" Google multicolore officiel, en SVG inline (pas de dépendance supplémentaire).
function GoogleGIcon({ size = 18 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" aria-hidden="true">
      ...
      <path fill="#4CAF50" d="M24 44c5.5 0 10.4-1.9 14.3-5.1l-6.6-5.6C29.6 35.1 26.9 36 24 36c-5.3 0-9.7-3.1-11.3-7.9l-6.6 5.1C9.5 39.6 16.2 44 24 44z"/>
      <path fill="#1976D2" d="M43.6 20.5H42V20H24v8h11.3c-.8 2.3-2.3 4.2-4.2 5.6l6.6 5.6C39.7 37.4 44 31.4 44 24c0-1.3-.1-2.7-.4-3.5z"/>
    </svg>
  );
}
...
function AuthModal({ onClose, onAuthed }) {
  ...
  // Connexion/inscription via Google : Supabase gère tout le flow OAuth (redirect vers
  // Google, retour sur le site, création de session). Même trigger serveur handle_new_user()
  // que pour l'inscription email : si Google ne fournit pas de "pseudo" dans les métadonnées,
  // le trigger retombe automatiquement sur "Membre-xxxxxx", donc aucun changement DB requis.
  const signInWithGoogle = async () => {
    setErr(""); setInfo(""); setGoogleLoading(true);
    try {
      ...
      if (error) throw error;
      // Ici la page part vers Google : rien d'autre à faire, onAuthStateChange prendra
      // le relais au retour sur le site.
    } catch (e) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 70, padding: 16 }}>
      <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 16, maxWidth: 380, width: "100%", padding: 28 }}>
        ...
        <button onClick={signInWithGoogle} disabled={googleLoading || loading} style={{
          width: "100%", boxSizing: "border-box", display: "flex", alignItems: "center", justifyContent: "center", gap: 10,
          background: "#fff", color: "#3C4043", border: "1px solid #dadce0", borderRadius: 8, padding: "10px 14px",
          fontSize: 13, fontWeight: 600, cursor: googleLoading || loading ? "not-allowed" : "pointer", opacity: googleLoading || loading ? 0.7 : 1,
          marginBottom: 16,
        }}>
          <GoogleGIcon size={17} />
          {googleLoading ? "Redirection vers Google..." : mode === "login" ? "Continuer avec Google" : "S'inscrire avec Google"}
        </button>

        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
          <div style={{ flex: 1, height: 1, background: C.border }} />
          <span style={{ fontSize: 11, color: C.dim2, letterSpacing: 0.5 }}>OU</span>
          <div style={{ flex: 1, height: 1, background: C.border }} />
        </div>
